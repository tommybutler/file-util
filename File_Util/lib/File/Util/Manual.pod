...
We'll start out with some brief notes up front, and then the rest of this
document will cover the use of each feature in File::Util...
...
heirarchal datastructure of the file tree you ask it to search.

See the L<File::Util::Manual::Examples> for several useful ways to use
C<list_dir()>.

B<First, another note on L<SYNTAX|File::Util/SYNTAX>>:

B<To enable a flag, pass it in with the options hashref that must follow the>
B<initial directory name argument, and set the option to a true value (1).>

Syntax example to recursively return a list of subdirectories in
directory "dirname":

   $f->list_dir( 'dirname' => { dirs_only => 1, recurse => 1 } );
...
other options/flags to get the exact results you want in the datastructure.

=back

=item B<Searching and Matching with C<list_dir()>>

C<list_dir()> can use Perl L<Regular Expressions|perlre> to match against
the results it returns.  It can match based on file name, directory name,
the path preceeding results, and the parent directory of results.  The matching
arguments you use must be real regular expression references as shown
(i.e.- NOT strings).

Regular expressions can be provided as a single argument value, or a
specially crafted hashref designating a list of patterns to match against in
either an "or" manner, or an "and"ed cumulative manner.
...
Bear in mind that just because you tell <list_dir()> to match each directory
against the regex(es) you specify here, that doesn't mean you are telling it to
only show directories in its results.  You will get file names included in
the results as well, unless you combine this with the C<dirs_only> option.
...
=item Examples of matching with C<listdir()>
...
Multiple-argument matching examples with B<AND>
   ...
   # ... same concepts apply to "files_match" and "dirs_match" searches

B<***When you specify regexes for more than one search type parameter>, the
patterns are AND'ed together, as you'd expect, and all matching criteria must
...
(see L<perlre> for details on how they work.  The basic idea has already been
shown in some of the documentation here.  Use syntax like the regular
expression below to match anything that is NOT part of the subpattern.

   # match all files that are not *.MP3 files
   my @no_music = $f->list_dir(
      '/opt/music' => { files_match => qr/^(?!mp3$)/i }
   );
...
Recognized options are listed below.
   ...
   my $files_ref = $f->load_dir( $dirname );

      -OR-

   my $files = $f->load_dir( $dirname => { as_listref => 1 } );

      -OR-

   my @dir_files = $f->load_dir( $dirname => { as_list => 1 } );
...
If you want to load files into a hashref/listref or array while using the
...
and map the contents into your variable.
   ...
   %$hash_ref = map { $_ => $ftl->load_file( $_ ) }
                $ftl->list_dir( $dir_name => { advanced opts...} );
...
files opened by this method as well.  Adjust the readlimit as necessary.
   ...
   my $files = $f->load_dir( 'directory/to/load/' );
...
oct 777, B<combined with the user's current umask>.  If specified, the bitmask
...
of 0777, B<combined with the user's current umask>.
...
Example:
   ...
   $f->make_dir( '/home/jspice' => 644 => { if_not_exists => 1 } );
...
If a call to this method is made without the C<if_not_exists> flag and the
