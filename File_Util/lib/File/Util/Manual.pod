...
via the B<C<onfail>> option.  If you are not sure if a given method accepts an
"options hashref", just keep reading -- each method is covered in detail
in the L<METHODS|/METHODS> section below.

I<An explanation of the "options hashref":>

   # "hashref" is short for "hash reference".  Hash references look like:
   my $hashref = { name => 'Larry', language => 'Perl', pet => 'Velociraptor' };

   # File::Util often uses hash references as arguments to allow you to enable
   # or disable certain features or behaviors, so you get the output you want.
...
for everything and never have to worry about it after that.
   ...
   # every error will cause a warning instead of dying with an error:
   my $ftl = File::Util->new( { onfail => 'warn' } );

   $ftl->write_file( 'C:\\' => 'bogus' ); # now this call will warn and not die
