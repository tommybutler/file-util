...
then we'll talk about custom error handling in File::Util, and finally the rest
of this document will cover File::Util's object methods, one by one, with brief
usage examples.
...
options.  In fact, every single call to a File::Util method can include
an error handling directive.  File::Util has some pre-defined error handling
behaviors that you can choose from, or you can supply your own error handler
routine.  This is accomplished via the B<C<onfail>> option.

As an added convenience, when you use this option with the
L<File::Util constructor method|/new>, it sets the default error
handling policy for all failures; in other words, you can set up one error
handler for everything and never have to worry about it after that.
...
Example:
   ...
   File::Util->load_file( ... { onfail => 'zero' } );
...
Example:
   ...
   File::Util->write_file( ... { onfail => 'warn' } );
...
Example:
   ...
   File::Util->list_dir( ... { onfail => 'message' } );
...
Examples:
   ...
   File::Util->new( { onfail => \&politician_error_handler } );

      -OR-

   # Define and apply your error handler in one step:

   File::Util->new( { onfail => sub { my ( $err, $stack ) = @_; ... } } );
...
directory "dir_name":
   ...
   $f->list_dir( 'dir_name' => { dirs_only => 1, recurse => 1 } );
...
B<*NOTE:> Bear in mind that just because you tell <list_dir()> to match each
...
Recognized options are listed below.
   ...
   my $files_ref = $f->load_dir( $dirname ); # default (hashref)

      -OR-

   my $files_ref = $f->load_dir( $dirname => { as_listref => 1 } );
...
Acceptable values are all covered in the L<ERROR HANDLING|/ERROR HANDLING>
...
=item I<Syntax:> C<< open_handle( file => [file name] => { options } ) >>
