...
something goes wrong in your program using File::Util for a given file
operation.  The error messages are diagnostic in nature and provide not only
information about what went wrong, but also hints on how to fix it.

These error messages can be turned off in production code, by customizing your
call to the constructor.  See L<DIAGNOSTICS|/DIAGNOSTICS> for the details.
...
possible about the error.  In C<File::Util>, you incurr no performance penalties
by enabling more verbose error messages.  In fact, you're encouraged to do so.

You can enable diagnostics globally (for every C<File::Util> object you
...
=item Enable Diagnostics globally: C<use File::Util qw( :diag )>

=item Enable Diagnostics per-object: C<< File::Util->new( diag => 1 ) >>

=item Enable Diagnostics per-call: C<< $ftl->load_file( 'abc.txt' => { diag => 1 } ) >>
